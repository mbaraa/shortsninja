...
// or the IP is not registered, it returns a dummy user with a shorts ninja logo :)
func (um *UserManager) getUserFromDB(sess *models.Session) *models.User {
	session, _ := um.db.GetSession(sess)
	if session == nil {
		return um.getDummyUser()
	}
	user, _ := um.db.GetUser(&models.User{Email: session.UserEmail})
	if user == nil {
		return um.getDummyUser()
	}
