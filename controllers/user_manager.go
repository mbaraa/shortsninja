...
// GetUserFromToken returns a user from the database using an existing and valid token
func (um *UserManager) GetUserFromToken(req *http.Request) *models.User {
	token := ""
	callerIP := um.reqData.GetIP(req)

	if token = um.getTokenFromCookie(req); token != "" {
	} else if token = um.getTokenFromQuery(req); token != "" {
	}

	return um.getUserFromDB(token, callerIP)
}
...
func (um *UserManager) SetToken(w http.ResponseWriter, r *http.Request) {
	if token := um.getTokenFromQuery(r); token != "" {
...
// getTokenFromQuery returns token from the the request query
func (um *UserManager) getTokenFromQuery(req *http.Request) string {
	if token := req.URL.Query()["token"]; token != nil {
		return token[0]
	}
	return ""
}
